---
title: "Covid-19 Vaccinations - Trends and Adverse Effects"
...
### Introduction
Covid-19 has been on everyone's mind since it originally came to the United States in the earlier months of 2020. Since that time, medical experts began working fervently to create a vaccine in order to help keep the public safe from the pandemic and to help slow the spread of this virus. In late 2020 and early 2021, the first vaccines finished all clinical trials and phases of testing, at which point distribution began. The Covid-19 vaccines are groundbreaking because they are the fastest vaccines ever created. We decided to look at data regarding the COVID vaccines because this is the newest development in the pandemic, and as many individuals are getting the vaccine, there are concerns about experiencing adverse symptoms after receiving a dose. We specifically wanted to analyze general vaccine information, looking at the distribution of the vaccines and trends in the amount of vaccines administered and the timeline of administration. We also wanted to look into the symptoms that the COVID shots have been causing to see if we could discover any trends or correlations between the symptoms and various factors such as age and previous medical history.
...
datatable(topPerState)
```


To begin answering this question, we joined the Vaccine dataset with the Data dataset in order to be able to summarize by the amount of each vaccine distributed in each state. After creating that dataframe, we could sort and create a new dataframe that includes only the top most frequenly distributed vaccine in each state. The results are shown in the above table.
...
In addition to learning about the specific symptoms after receiving the vaccine based on the manufacturer and further analyzing the effects of age on experiencing symptoms, we wanted to discover whether there were common trends between a patient's medical history and symptoms that they experienced with the vaccine.
...
ggplot(deathAfterVaxSex,aes(x=SEX,y=n)) + geom_col(fill = 'steelblue') + ggtitle("Deaths after Vaccine by Gender") + xlab("Gender") + ylab("Number of Deaths after receiving a vaccine")
```
...
#### **Julia Lundstrum**
  I helped find and retrieve the data sets for us to use in our analysis. I also helped clean our data sets and combined them. I then came up with additional questions for us to explore in our analysis. I wrote the section about our data sets. I performed the analysis on the correlation between total Covid-19 cases and fully vaccinated people per state, also looking at possible reasonings behind the correlation. My commits can be seen on the Github repo by my username JL496.

#### **Dana Thacker**
  My contributions to this project included helping to select the topic of Covid-19 Vaccine data and discussing with group members to decide specifically what questions to analyze. The questions I analyzed in RStudio were the distribution of vaccines among states and the relationship between a patient's medical history and their post-vaccine symptoms. I also played a large role in coordinating meetings outside of class via Zoom for our group to discuss our progress on the project. Finally, I compiled all of the group members pre-recorded videos discussing their questions for the presentation and created the final mp4 file that was presented to the class. All of my specific code and commits can be found on the Github repo by my username thackerdana.
...
#### **Andrew Fahmy**
  My contributions to the project was exploring the data to see how the number of reports of adverse effects from the Covid-19 vaccine trended by state and nation wide. This included preparing the data and getting it to a state so I could further analyze this question. Once I had the data ready, I created graphs and chloropleth maps in order to show the trends and have a nice visulization for the audience. Lastly, I talked about the different challenges that we had faced throughout this project and how we were able to over come them. You can see the history of my work on the Github repo, my username is ajfahmy.
