...
#Now I want to filter each of them so that I only have the top symptom per medical history thing for each dataframe, then compare those dataframes to see the top symptom for each medical history thing
```
...
#str(vaxinfo_daily_state)

#vaxinfo_daily %>%
#    ggplot(aes(x = VAX_DATE, y = DAILY_TOTAL)) +
#    geom_line()
...
vaxinfo_daily_state %>%
    filter(VAX_DATE > as.Date("2020-12-01")) %>%
...
ggplot(dat, aes(x = long, y = lat)) +
    geom_polygon(aes(group = group, fill = TOTAL), olor = "white", size = 0.1) +
    coord_map() +
